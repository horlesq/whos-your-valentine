...
const CreatorPage = () => {
  ...
  const handleSubmit = async () => {
    if (images.length !== 9 || !senderName.trim()) {
        alert("Please upload exactly 9 images and enter your name.");
        return;
    }

    setIsUploading(true);
    try {
      // PROD: `/.netlify/functions/create-valentine`
      // DEV: `http://localhost:8888/.netlify/functions/create-valentine` (if using netlify dev)
      // For now, we assume relative path works if deployed or proxied
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-100 via-red-100 to-rose-200 p-4 md:p-8 flex items-center justify-center font-[Outfit]">
      ...
      >
        <div className="bg-gradient-to-r from-pink-500 to-rose-500 p-6 text-white text-center">
            <h1 className="text-3xl font-bold font-['Great_Vibes']">Create Your Valentine</h1>
            <p className="opacity-90 mt-2 text-sm">Upload 9 photos to create a custom game for your love.</p>
        </div>

        <div className="p-6 md:p-8 space-y-8">
            {/* Step 1: Name */}
            <div className="space-y-2">
                <label className="block text-sm font-medium text-gray-700">1. Who is this from?</label>
                <input 
                    type="text" 
                    value={senderName}
            ...
            <div className="space-y-4">
                 <div className="flex justify-between items-center">
                    <label className="block text-sm font-medium text-gray-700">2. Upload Photos ({images.length}/9)</label>
                    {images.length < 9 && (
                 ...
                 <div className="grid grid-cols-3 gap-2">
                    ...
                    {Array.from({ length: Math.max(0, 9 - images.length) }).map((_, i) => (
                        <div key={`empty-${i}`} className="aspect-square rounded-lg border-2 border-dashed border-gray-200 flex items-center justify-center text-gray-300">
                            <Heart size={24} />
                        </div>
                    ))}
                 </div>
            ...
            {generatedLink ? (
                <div className="bg-green-50 border border-green-100 rounded-xl p-6 text-center space-y-4 animate-in fade-in zoom-in duration-300">
                    ...
                    <div>
                        <h3 className="text-lg font-bold text-gray-900">Your Valentine is Ready!</h3>
                        <p className="text-gray-500 text-sm">Share this special link with your person.</p>
                    </div>
                    
                    <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg p-2 pl-4">
                        <input readOnly value={generatedLink} className="flex-1 text-sm text-gray-600 outline-none bg-transparent" />
                        <button 
                    ...
                    <a href={generatedLink} target="_blank" rel="noreferrer" className="inline-block text-sm text-pink-600 font-medium hover:underline decoration-pink-300 underline-offset-4">
                        Open Link to Test ->
                    </a>
                </div>
            ) : (
                <button
                    ...
                    disabled={images.length !== 9 || !senderName || isUploading}
                ...
                >
                    {isUploading ? (
                        ...
                        <>Create Valentine <Heart size={20} fill="currentColor" /></>
                    )}
                </button>
            )}
        </div>
